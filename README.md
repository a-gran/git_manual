...
1. [git init](#1-git-init) - Инициализирует новый Git-репозиторий в текущей директории
2. [git add](#2-git-add) - Добавляет изменения в текущей директории и вложенных папках в индекс (staging area)
3. [git commit](#3-git-commit) - Фиксирует изменения из индекса с сообщением коммита
4. [git push](#4-git-push) - Отправляет локальные изменения в удаленный репозиторий
5. [git pull](#5-git-pull) - Получает изменения из удаленного репозитория и сливает их в текущую ветку
6. [git remote](#6-git-remote) - Добавляет удаленный репозиторий, отображает список удаленных, переименовывает
7. [git branch](#7-git-branch) - Показывает список веток, создает новую ветку и проверяет её наличие
8. [git fetch](#8-git-fetch) - Получает последние данные из удаленного репозитория, но не вносит их в текущую рабочую директорию
9. [git checkout](#9-git-checkout) - Переключается на указанную ветку
10. [git merge](#10-git-merge) - Объединяет указанную ветку с текущей
11. [git status](#11-git-status) - Показывает текущее состояние репозитория, включая незафиксированные изменения
12. [git reset](#12-git-reset) - Откатывает текущую ветку к указанному коммиту
13. [git clone](#13-git-clone) - Клонирует (копирует) удаленный репозиторий на локальный компьютер
14. [git log](#14-git-log) - Показывает историю коммитов в репозитории
15. [git diff](#15-git-diff) - Показывает различия между коммитами, ветками или рабочей директорией
16. [git stash](#16-git-stash) - Временно сохраняет незакоммиченные изменения для последующего восстановления
17. [git tag](#17-git-tag) - Создает, просматривает и удаляет теги (метки) для важных точек в истории
18. [git rebase](#18-git-rebase) - Переносит коммиты текущей ветки на другую базу
19. [git cherry-pick](#19-git-cherry-pick) - Применяет изменения из конкретного коммита к текущей ветке
20. [git revert](#20-git-revert) - Создает новый коммит, отменяющий изменения указанного коммита
21. [git rm](#21-git-rm) - Удаляет файлы из рабочей директории и индекса Git
22. [git mv](#22-git-mv) - Перемещает или переименовывает файлы и автоматически индексирует изменения
23. [git show](#23-git-show) - Показывает подробную информацию о Git-объекте (коммите, теге, дереве)
24. [git config](#24-git-config) - Управляет настройками Git (локальными и глобальными)
25. [git blame](#25-git-blame) - Показывает, кто и когда изменил каждую строку файла
26. [git clean](#26-git-clean) - Удаляет неотслеживаемые файлы из рабочей директории
27. [git reflog](#27-git-reflog) - Показывает историю всех изменений указателя HEAD
28. [git switch](#28-git-switch) - Переключается между ветками (современная альтернатива git checkout)
29. [git restore](#29-git-restore) - Восстанавливает файлы в рабочей директории
30. [git shortlog](#30-git-shortlog) - Показывает сводную статистику коммитов по авторам
31. [git grep](#31-git-grep) - Ищет текст в файлах репозитория, включая всю историю коммитов
32. [git bisect](#32-git-bisect) - Использует бинарный поиск для нахождения коммита, который внес ошибку
33. [git submodule](#33-git-submodule) - Управляет подмодулями — вложенными репозиториями внутри основного
34. [git worktree](#34-git-worktree) - Позволяет работать с несколькими рабочими директориями одновременно
35. [git archive](#35-git-archive) - Создает архив файлов из репозитория без Git-метаданных
36. [git describe](#36-git-describe) - Показывает самый близкий тег к текущему коммиту с дополнительной информацией
37. [git notes](#37-git-notes) - Добавляет заметки к коммитам без изменения самих коммитов
38. [git apply](#38-git-apply) - Применяет патч к файлам в рабочей директории
39. [git format-patch](#39-git-format-patch) - Создает патчи в формате email для отправки коммитов
40. [git gc](#40-git-gc) - Очищает и оптимизирует локальный репозиторий (garbage collection)
